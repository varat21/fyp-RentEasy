...
export const useFormValidation = () => {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      phoneNumber: (value) => value && value.length < 10 ? '* Phone must have at least 10 numbers' : null,
      address: (value) => (value?.trim() === '' ? '* Address is required' : null),
      email: (value) =>/^\S+@\S+$/.test(value) ? null : '* Enter a valid email address',
      password: (value) =>value && value.length < 6 ? '* Password must have at least 6 characters' : null,
